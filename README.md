...
| 👥 **จัดการสมาชิก** | แอดมินดูรายชื่อและปิดใช้งานสมาชิกแบบ Soft Delete โดยไม่ลบประวัติลงเวลา |
...
การแก้ไข GPS, การดูสถานะ LINE Bot และการเปิด/ปิดโหมดรับลงทะเบียนต้องผ่านการล็อกอินผู้ดูแล โดยตรวจสิทธิ์ซ้ำที่ Google Apps Script ฝั่งเซิร์ฟเวอร์ ส่วนพนักงานไม่ต้อง Login และลงทะเบียนได้เฉพาะช่วงที่แอดมินเปิดรับ
...
6. เมื่อต้องการรับพนักงานใหม่ ให้กำหนดระยะเวลาแล้วกด **เปิดรับลงทะเบียน** จากนั้นคัดลอกลิงก์ส่งให้พนักงาน
7. กด **"บันทึกการตั้งค่าทั้งหมด"**
...
1. แอดมิน Login ที่ `/config.html` แล้วเปิด **โหมดรับลงทะเบียน** ตามเวลาที่ต้องการ (5–480 นาที)
2. แอดมินกด **คัดลอกลิงก์ลงทะเบียน** และส่งให้พนักงาน
3. พนักงานเปิด `/register.html` ได้โดยไม่ต้อง Login ระบบจะอนุญาตเฉพาะช่วงที่เปิดรับ
4. กรอกชื่อ-นามสกุลพนักงาน
5. มองกล้อง **ตรงๆ** แล้วกด **"บันทึกใบหน้า"** (ครั้งที่ 1)
6. **หันซ้ายเล็กน้อย** กด **"บันทึกเพิ่ม"** (ครั้งที่ 2)
7. **หันขวาเล็กน้อย** กด **"บันทึกเพิ่ม"** (ครั้งที่ 3)
...
| `Unexpected token '<'` หรือ `DOCTYPE is not valid JSON` | หน้าเว็บได้รับ HTML แทน JSON ระบบเวอร์ชันใหม่จะลองซ้ำและแสดงข้อความที่อ่านง่าย ตรวจว่า Apps Script Web App เปิดสิทธิ์เป็น **Anyone** แล้วรีเฟรชหน้า |
...
หน้า Scan ใช้ Tiny Face Detector สำหรับมือถือ โหลด GPS/โมเดล/รายชื่อพนักงานแบบขนาน และไม่รัน AI ซ้อนกันระหว่างรอบสแกน ส่วน Apps Script cache รายชื่อพนักงานไว้ 5 นาทีและล้าง cache ทันทีเมื่อมีการลงทะเบียนใหม่

ข้อความ LINE ใช้วันที่ภาษาไทย เช่น `22 ก.ค. 2569` และขาออกจะแสดงเวลาทำงานในรูปแบบชั่วโมงและนาที เช่น `8.38 ชม.`
